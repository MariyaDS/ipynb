...
# 
# The first time the data type of employee_id is displayed, it appears as an integer (<class 'int'>). After reassigning employee_id to a string format using the str() function, the data type of employee_id becomes a string (<class 'str'>).
# 
...
# This code outputs ".com" because it uses string slicing to extract the substring starting from the index where ".com" begins (stored in the ind variable) and ending at ind + 4, which includes the characters ".com".
# 
# 
...
# 
# 
# In these exercises, we covered various string manipulation techniques in Python, focusing on tasks relevant to cybersecurity. We learned how to convert data types, extract specific characters from strings, find the index of substrings, and slice strings to extract relevant information from URLs. These skills are crucial for tasks such as parsing log files, analyzing network traffic, and processing data in cybersecurity applications. By practicing these exercises, we gained a solid foundation in string manipulation, which is essential for handling and processing data effectively in cybersecurity scenarios.
#     
# 
# 
# 
